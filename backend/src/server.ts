...
import { seedWarehouseMap } from './mockData';
import { findShortestPath } from './router';
import { 
  Package, 
  PackageType, 
  SimulationStats, 
...
const warehouseMap = seedWarehouseMap();
...
function spawnPackage(
  ...
  // Route calculation
  const route = findShortestPath(warehouseMap, sourceNode.id, targetNode.id);
 ...
 */
function recalculateActiveRoutes(): void {
  for (const pkg of packages) {
    ...
    const newRoute = findShortestPath(warehouseMap, currentNodeId, pkg.targetNodeId);
  ...
  for (const pkg of packages) {
    ...
    if (pkg.progress >= 1.0) {
      ...
      if (!updatedNextNodeId) {
        ...
        const alternateRoute = findShortestPath(warehouseMap, updatedCurrentNodeId, pkg.targetNodeId);
